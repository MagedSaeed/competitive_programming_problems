...
public class P101 {
    ...
    static ArrayList<LinkedList<Integer>> blockTable;
    ...
    public static void main(String  args[]) {
        ...
        blockTable = new ArrayList<>(blocks);
...
//            System.out.println(line);
            ...
            if(splitLine.length>1) {
                String command = splitLine[0] + " " + splitLine[2];
                int a = Integer.parseInt(splitLine[1]);
                int b = Integer.parseInt(splitLine[3]);
//                System.out.println(command+" "+a+" "+b);
                switch (command) {
                    case "move onto":
                        moveOntoOver(a, b, 0);
                        break;
    ...
    private static void moveOntoOver(int a, int b, int type){

        int placeA = findPlace(a);
        int placeB = findPlace(b);
        if(placeA != placeB) {
            extractStackedBlocks(placeA, a);
            if(type == 0)
                extractStackedBlocks(placeB, b);
            blockTable.get(placeB).add(a);
            blockTable.get(placeA).remove(Integer.valueOf(a));
        }
    }

    private static void pileOntoOver(int a, int b, int type){
        int placeA = findPlace(a);
        int placeB = findPlace(b);
        if(placeA != placeB) {
            if (type == 0)
            ...
            for (int i = blockTable.get(placeA).indexOf(a); i < blockTable.get(placeA).size(); )
                pile.add(blockTable.get(placeA).remove(i));

            for (Integer el : pile)
                if (!blockTable.get(placeB).contains(el))
                    blockTable.get(placeB).add(el);
        }
    }

    private static void extractStackedBlocks(int place, int el) {
        if(blockTable.get(place).size()>1){
            int i = blockTable.get(place).size()-1;
            // return blocks to their original places.
            while(blockTable.get(place).get(i) != el){
                // check if the original place contains the element.
                if(!blockTable.get(blockTable.get(place).get(i)).contains(blockTable.get(place).get(i))) {
                    int curEl = blockTable.get(place).remove(i);
                    blockTable.get(curEl).add(curEl);
                }
                else{
                    blockTable.get(blockTable.get(place).remove(i));
                }
                i--;
            }
        }
    ...
    private static int findPlace(int el){
        int place = -1;
        for (LinkedList<Integer> block : blockTable)
            if(block.contains(el))
                place = blockTable.indexOf(block);
