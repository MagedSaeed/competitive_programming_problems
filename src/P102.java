...
public class P102 {
    public static void main(String[] args) {
        ...
        while(sc.hasNextLine()){
            for(int i=0; i<9; i++){
                if(i/3 == 0)
                    list1[i/3+i%3] = sc.nextInt();
                else if(i/3 == 1)
                    list2[i/3+i%3-1] = sc.nextInt();
                else
                    list3[i/3+i%3-2] = sc.nextInt();
            }

            Map<String, Integer> dict = new HashMap<>();
            dict.put("BGC", list2[0]+list3[0]+list1[1]+list3[1]+list1[2]+list2[2]);
            dict.put("BCG", list2[0]+list3[0]+list1[2]+list3[2]+list1[1]+list2[1]);
            dict.put("GBC", list2[1]+list3[1]+list1[0]+list3[0]+list1[2]+list2[2]);
            dict.put("GCB", list2[1]+list3[1]+list1[2]+list3[2]+list1[0]+list2[0]);
            dict.put("CBG", list2[2]+list3[2]+list1[0]+list3[0]+list1[1]+list2[1]);
            dict.put("CGB", list2[2]+list3[2]+list1[1]+list3[1]+list1[0]+list2[0]);

            Map<String, Integer> sortedDict = sortByValue(dict);
            Integer min = (Integer)sortedDict.values().toArray()[0];
            Map<String, Integer> minKeys = new TreeMap<>();
            for(String key: sortedDict.keySet())
                if(sortedDict.get(key).equals(min))
                    minKeys.put(key, sortedDict.get(key));
                else
                    break;
            System.out.println(minKeys.keySet().toArray()[0]+" "+minKeys.values().toArray()[0]);
            if(!sc.hasNextInt())
                break;
        }
    }
    ...
    private static Map<String, Integer> sortByValue(Map<String, Integer> unsortMap) {
        ...
        //    new syntax proposed by the compiler
